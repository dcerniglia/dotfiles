...
function activate(context) {
    var isDisabledByDefault = vscode.workspace.getConfiguration("tabout").get('disableByDefault');
    context.workspaceState.update("tabout-active", (isDisabledByDefault ? false : true));
    context.subscriptions.push(vscode.commands.registerCommand('toggle-tabout', function () {
        var currentState = context.workspaceState.get("tabout-active");
    ...
    var tabout = vscode.commands.registerCommand('tabout', function () {
        ...
        var currentLineText = editor.document.lineAt(editor.selection.active.line).text;
        var currentPositionInLine = editor.selection.active.character;
        ...
        var previousCharacter = utils_1.getPreviousChar(currentPositionInLine, currentLineText);
        var characterInfo = charactersToTabOutFrom_1.characterSetsToTabOutFrom().find(function (o) { return o.open == previousCharacter || o.close == previousCharacter; });
        if (characterInfo !== undefined) {
            var nextCharacter_1 = utils_1.getNextChar(currentPositionInLine, currentLineText);
            var indxNext = charactersToTabOutFrom_1.characterSetsToTabOutFrom().find(function (o) { return o.open == nextCharacter_1 || o.close == nextCharacter_1; });
        ...
        if (characterInfo !== undefined) {
            ...
            var positionNextSpecialCharacter = utils_1.determineNextSpecialCharPosition(characterInfo, currentLineText, currentPositionInLine);
            if (positionNextSpecialCharacter > -1) {
                //Move cursor
                var nextCursorPosition = new vscode.Position(editor.selection.active.line, positionNextSpecialCharacter);
